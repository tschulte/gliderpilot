 ...
 * @author tobias
 */
public final class GeoCoordinateImpl implements GeoCoordinate {
     ...
     */
    public GeoCoordinateImpl(double pLat, double pLon) {
        _lat = Quantity.valueOf(pLat, DEGREE_ANGLE);
        _lon = Quantity.valueOf(pLon, DEGREE_ANGLE);
    }

    /**
     * Construct a new GeoCoordinate for the given latitude and longtude.
     */
    public GeoCoordinateImpl(Angle pLat, Angle pLon) {
        _lat = pLat;
        _lon = pLon;
    }
    ...
    public boolean isValid() {
        return _lat != null && _lon != null && _lat.abs().compareTo(MAX_LAT) <= 0
                && _lon.abs().compareTo(MAX_LON) <= 0;
    }
    ...
    public Length getDistanceTo(GeoCoordinate pCoord) {
        ...
        GeoCoordinateImpl coord = (GeoCoordinateImpl) pCoord;
        Angle lat1 = getLat();
    ...
    public Angle getAverageCourseTo(GeoCoordinate pCoord) {
        ...
        Angle trueCourseInitial = trueCourseTo(pCoord);
        Angle trueCourseEnd = Course.inverse(pCoord.trueCourseTo(this));
